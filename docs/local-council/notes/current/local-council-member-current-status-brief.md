...
- 문서 surface: `shared`
- 주요 독자: `현직 지방의원 frontend 범위와 제약을 빠르게 파악해야 하는 팀원`
- 상태: `active`
- 관련 PR: `#24`, `#33`
- 정본 여부: `아니오`
...
- 지금 화면이 어디까지 구현됐는지 5분 안에 파악하려면 이 문서를 읽는다.
- 브라우저에서 직접 확인하려면 [frontend/backend 로컬 확인 가이드](../../runbooks/local-frontend-backend-check-guide.md)를 읽는다.
- Playwright 자동 검증을 돌리려면 [Playwright E2E 실행 가이드](../../runbooks/playwright-e2e.md)를 읽는다.

#### agent 기준

- agent가 작업 경로를 잡으려면 [현직 지방의원 도메인 LLM 진입 문서](../../canonical/llm-entry.md)를 먼저 읽는다.

## 이 문서의 역할

- 이 문서는 `현직 지방의원` frontend의 현재 구현 범위와 계약 경계를 빠르게 공유하는 current status brief다.
- 화면 흐름과 데이터 경계만 요약하고, 실제 실행 절차와 긴 명령 목록은 runbook으로 보낸다.
- backend 계약이나 제품 의도를 함께 보려면 아래 문서를 같이 본다.
...
- 주요 사용자 흐름은 `/local-council`의 `주소 선택 -> resolve -> roster -> person detail`이다.
- 브라우저는 backend를 직접 호출하지 않고 Next route를 통해 proxy한다.
- backend가 없거나 `WOOGOOK_BACKEND_BASE_URL`이 비어 있으면 강동구 샘플 fixture로 fallback한다.
- 사람이 직접 확인할 때는 `frontend-only sample mode`와 `backend-connected live mode`를 둘 다 본다.
- 수동 검증 절차와 명령은 `docs/local-council/runbooks/local-frontend-backend-check-guide.md`에만 유지한다.
...
- 식별자 계약
  - `basic_council` `person_key`는 opaque key로만 소비한다.
...
- 확인 포인트
  - backend 없이도 `/local-council` 주소 입력부터 roster, detail, overlay sample까지 끝까지 확인할 수 있어야 한다.
...
- 자세한 절차와 체크리스트는 `docs/local-council/runbooks/local-frontend-backend-check-guide.md`를 따른다.
