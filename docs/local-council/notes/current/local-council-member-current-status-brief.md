...
- 따라서 아래 항목 중 `강동구 운영 안정화`는 V1 마감 범위이고, 나머지는 V1 이후 확장이다.
- V1을 완료라고 부르려면 live/sample smoke, proxy smoke, 제한 안내, degraded diagnostics 해석이 문서 기준대로 반복 재현돼야 한다.
- 후속 구현은 `docs/superpowers/plans/2026-04-12-gangdong-productization-slice-frontend-plan.md`를 기준으로 본다.
- 현재 기준 frontend 다음 단계는 아래 셋이다.
  - 강동구 운영 안정화
    - live/sample smoke와 proxy smoke를 계속 고정한다.
    - diagnostics/freshness copy를 사람이 읽기 쉽게 다듬는다.
  - 설명력 보강
    - summary, evidence, freshness, diagnostics 문구를 더 제품 친화적으로 정리한다.
    - `구청장 + 구의원` 명단 표현과 상세 안내 문구를 계속 점검한다.
  - 서울 전역 확장 준비
