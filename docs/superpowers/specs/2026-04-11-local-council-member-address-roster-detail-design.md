...
Next route는 `WOOGOOK_BACKEND_BASE_URL` 기반으로 FastAPI에 proxy한다. `person_key`는 `encodeURIComponent`로 path segment 안전성을 확보한다.
...
- `WOOGOOK_BACKEND_BASE_URL` 없음 또는 backend 통신 실패: `현직자 조회가 아직 준비되지 않았습니다. 잠시 후 다시 시도해주세요.`
...
- 화면 흐름 검증: 주소 제출 후 roster 표시, 인물 클릭 후 detail 표시, 404 안내 문구를 확인한다.
...
2. `local-council` zod schema와 API client 함수를 추가한다.
3. Next local-council proxy route를 추가한다.
4. `/local-council` page와 address, roster, detail view를 추가한다.
5. 서비스 허브에 `현직 지방의원` 진입 카드를 추가한다.
6. 테스트와 lint/build 검증을 수행한다.
