...
export function resolveSectionActionLink({
  ...
  const directViewUrl = sanitizeExternalUrl(item.source_url);
  ...
  return {
    viewUrl:
      directViewUrl ??
      sanitizeExternalUrl(itemSourceRef.source_url) ??
      sanitizeExternalUrl(matchedSectionRef?.source_url) ??
      sanitizeExternalUrl(preferredSectionRef?.source_url),
    downloadUrl:
      directDownloadUrl ??
      sanitizeExternalUrl(itemSourceRef.download_url) ??
      sanitizeExternalUrl(matchedSectionRef?.download_url) ??
      sanitizeExternalUrl(preferredSectionRef?.download_url),
  };
}
