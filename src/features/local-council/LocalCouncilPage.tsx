...
type LocalCouncilHistoryState = {
  ...
  district?: string;
  dong?: string;
  localCouncilHistoryDepth?: number;
};

function getLocalCouncilHistoryDepth(value: unknown) {
  if (!value || typeof value !== "object") {
    return 0;
  }

  const record = value as Record<string, unknown>;
  const depth = record.localCouncilHistoryDepth;
  return Number.isInteger(depth) && Number(depth) >= 0 ? Number(depth) : 0;
}
...
function getHistoryState(value: unknown): LocalCouncilHistoryState {
  ...
  return {
    ...
    district,
    dong,
    localCouncilHistoryDepth: getLocalCouncilHistoryDepth(record),
  };
}
...
export function canUseLocalCouncilHistoryBack(
  rawState: unknown,
  historyLength: number,
) {
  return (
    hasViewState(getHistoryState(rawState)) &&
    getLocalCouncilHistoryDepth(rawState) > 0 &&
    historyLength > 1
  );
}
...
export default function LocalCouncilPage() {
  ...
  const pushView = (nextView: View, historyState?: LocalCouncilHistoryState) => {
    const nextHistoryState = {
      ...(historyState ?? createHistoryState(nextView)),
      localCouncilHistoryDepth:
        historyState?.localCouncilHistoryDepth ??
        getLocalCouncilHistoryDepth(window.history.state) + 1,
    };
    viewRef.current = nextView;
    setView(nextView);
  ...
  const replaceView = (nextView: View, historyState?: LocalCouncilHistoryState) => {
    const nextHistoryState = {
      ...
      localCouncilHistoryDepth:
        historyState?.localCouncilHistoryDepth ??
        getLocalCouncilHistoryDepth(window.history.state),
    };
    viewRef.current = nextView;
    setView(nextView);
  ...
  useEffect(() => {
    ...
    if (!getCurrentHistoryState(window.history.state).view) {
      window.history.replaceState(
        buildBrowserHistoryState({
          ...createHistoryState("address"),
          localCouncilHistoryDepth: 0,
        }),
        "",
  ...
  const handleRosterBack = () => {
    ...
    cancelDetailRequest();
    cancelHistoryRestoreRequest();
    if (canUseLocalCouncilHistoryBack(window.history.state, window.history.length)) {
      window.history.back();
      return;
    }
    replaceView("address", createHistoryState("address"));
