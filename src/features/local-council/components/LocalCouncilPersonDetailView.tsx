...
function buildProfilePhotoProxyUrl(pageUrl: string) {
  const params = new URLSearchParams({ pageUrl });
  return `/api/local-council/v1/profile-photo?${params.toString()}`;
}

function sanitizeNavigableHref(value: string) {
...
function InteractivePillLink({
  ...
  const safeHref = sanitizeNavigableHref(href);
  if (!safeHref) {
    return <StaticPill label={label} />;
  }
...
function HeroMetaList({
  ...
  return (
    <div className="grid gap-1.5 sm:grid-cols-[76px_minmax(0,1fr)] sm:gap-3">
      <p className="text-sm font-semibold" style={{ color: "var(--navy)" }}>
        {label}
      </p>
      <ul className="grid gap-1 text-sm leading-6" style={{ color: "var(--text-secondary)" }}>
        {items.map((item) => (
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {item.sourceLabel ? (
                            item.sourceUrl ? (
                              <InteractivePillLink
                                href={item.sourceUrl}
                                label={`출처 · ${item.sourceLabel}`}
                              />
                            ) : (
                              <StaticPill label={`출처 · ${item.sourceLabel}`} />
                            )
                          ) : item.sourceUrl ? (
                            <InteractivePillLink href={item.sourceUrl} label="출처" />
                          ) : null}
                          {hasViewAction && item.actions.viewUrl ? (
                            <InteractivePillLink
                              href={item.actions.viewUrl}
                              label={item.actions.viewLabel ?? "원문 보기"}
                            />
                          ) : null}
                          {hasDownloadAction && item.actions.downloadUrl ? (
                            <InteractivePillLink
                              href={item.actions.downloadUrl}
                              label={getLocalCouncilDownloadActionLabel(item.actions.downloadLabel)}
                            />
                          ) : null}
                        </div>
                        {hasRelatedSourceLinks ? (
...
function PersonHeroAvatar({
  ...
  const safeProfilePageUrl = profilePageUrl
    ? sanitizeNavigableHref(profilePageUrl)
    : null;
  const safeImageUrl = imageUrl ? sanitizeNavigableHref(imageUrl) : null;
  const imageCandidates = Array.from(
    new Set(
      [safeProfilePageUrl ? buildProfilePhotoProxyUrl(safeProfilePageUrl) : null, safeImageUrl].filter(
  ...
  const hasProfileImage = Boolean(activeImageUrl);

  return (
    <div
      className="relative flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-lg text-2xl font-bold"
      style={{
    ...
    >
      ...
      {hasProfileImage && activeImageUrl ? (
        <>
          {/* External profile photos can require session-based proxying; keep plain img local instead of Next Image. */}
            ...
            >
              ...
              <div className="mt-4 grid gap-3">
                {section.items.map((item) => (
                  ...
                  >
                    {(() => {
                      const safeSourceUrl = item.sourceUrl
                        ? sanitizeNavigableHref(item.sourceUrl)
                        : null;
    ...
    <section className="mx-auto w-full max-w-5xl px-5 py-8">
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row">
          <PersonHeroAvatar
            key={`${hero.profilePageUrl ?? ""}:${hero.imageUrl ?? ""}:${hero.name}`}
            imageUrl={hero.imageUrl}
            profilePageUrl={hero.profilePageUrl}
            name={hero.name}
          />
          <div className="min-w-0 flex-1">
            ...
            {hero.educationItems.length > 0 || hero.careerItems.length > 0 ? (
              <div className="mt-3 grid gap-3">
                <HeroMetaList label="학력" items={hero.educationItems} />
                <HeroMetaList label="주요 약력" items={hero.careerItems} />
              </div>
            ) : null}
          </div>
        </div>
        {hero.links?.length ? (
          <div className="mt-3 flex flex-wrap gap-2">
            {hero.links.map((link) => (
              (() => {
                const safeHref = sanitizeNavigableHref(link.url);
                if (!safeHref) {
                  return (
                    <StaticPill key={`${link.label}:${link.url}`} label={link.label} />
                  );
                }
