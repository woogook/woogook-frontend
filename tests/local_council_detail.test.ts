...
test("buildPersonHeroMeta reads image, education, career, and links from flexible profile keys", () => {
  ...
  assert.equal(result.profilePageUrl, "https://example.com/profile");
...
test("LocalCouncilPersonDetailView keeps finance source CTA in expanded content without generic primary links", () => {
  ...
  assert.match(
    html,
    /href="https:\/\/www\.localfinance\.go\.kr\/finance\/gangdong\/budget-execution"[^>]*style="[^"]*background:var\(--navy\)[^"]*color:var\(--surface\)[^"]*"/,
  );
...
test("LocalCouncilPersonDetailView keeps the source badge non-clickable when backend sends an invalid placeholder URL", () => {
  ...
  assert.match(
    html,
    /<span[^>]*style="[^"]*background:var\(--surface-alt\)[^"]*color:var\(--text-secondary\)[^"]*"[^>]*>출처 · 지방재정365<\/span>/,
  );
  assert.doesNotMatch(html, /example\.invalid/);
  assert.doesNotMatch(html, /원문 보기/);
});

test("LocalCouncilPersonDetailView hides unsafe payload-driven overlay links", () => {
  const LocalCouncilPersonDetailView = loadLocalCouncilPersonDetailView({
    expandedKey: "overlay",
  });
  const html = renderToStaticMarkup(
...
test("LocalCouncilPersonDetailView keeps unsafe hero links non-clickable", () => {
  const LocalCouncilPersonDetailView = loadLocalCouncilPersonDetailView();
  const html = renderToStaticMarkup(
    createElement(LocalCouncilPersonDetailView, {
      person: {
...
test("LocalCouncilRosterView keeps only freshness copy in the roster header", () => {
  const LocalCouncilRosterView = loadLocalCouncilRosterView();
  const html = renderToStaticMarkup(
    createElement(LocalCouncilRosterView, {
      rosterData: {
  ...
  assert.doesNotMatch(html, /구청장 프로필 · 구청장 활동 외 3개 준비/);
  assert.doesNotMatch(html, /구청장은 구 행정을 총괄하는 단체장입니다\./);
  assert.doesNotMatch(html, /구의원은 구의회에서 조례와 예산, 감시 역할을 맡습니다\./);
});
...
test("LocalCouncilPersonDetailView removes the evidence digest section from the reader view", () => {
  const LocalCouncilPersonDetailView = loadLocalCouncilPersonDetailView();
  const html = renderToStaticMarkup(
    createElement(LocalCouncilPersonDetailView, {
      person: dossiers["seoul-gangdong:district-head"] as LocalCouncilPersonDossierResponse,
  ...
  assert.doesNotMatch(html, />근거 요약<\/h2>/);
  assert.doesNotMatch(html, /공식 프로필 1건/);
  assert.doesNotMatch(html, /요약 근거 출처/);
  assert.doesNotMatch(html, /강동구청장실 공식 프로필/);
  assert.doesNotMatch(html, /확인된 공식 근거를 바탕으로 핵심 활동과 출처를 정리했습니다\./);
...
test("LocalCouncilPersonDetailView uses the official profile photo proxy and stacked hero metadata for mobile readability", () => {
  const LocalCouncilPersonDetailView = loadLocalCouncilPersonDetailView();
  const html = renderToStaticMarkup(
    createElement(LocalCouncilPersonDetailView, {
      person: dossiers["seoul-gangdong:district-head"] as LocalCouncilPersonDossierResponse,
  ...
  assert.match(
    html,
    /src="\/api\/local-council\/v1\/profile-photo\?pageUrl=https%3A%2F%2Fwww\.gangdong\.go\.kr%2Fweb%2Fmayor%2Fcontents%2Fgdo010_010"/,
  );
  assert.doesNotMatch(html, /학력 ·/);
  assert.doesNotMatch(html, /주요 약력 ·/);
  assert.match(html, /<li[^>]*>이화여자대학교 사회과학대학 행정학과<\/li>/);
  assert.match(html, /<li[^>]*>연세대학교 행정대학원 도시행정학 석사<\/li>/);
  assert.match(html, /<li[^>]*>민선8기 강동구청장<\/li>/);
  assert.match(html, /<li[^>]*>제10대 서울특별시의회 의원<\/li>/);
});
